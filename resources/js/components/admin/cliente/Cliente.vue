  ...
  <div>
    ...
    <v-data-table :headers="headers" :items="desserts" sort-by="calories" class="elevation-1">
              ...
              <v-card-text>
                <v-container>
                  <v-row>
                    ...
                    <v-col cols="12" sm="6" md="4">
                      <v-text-field v-model="editedItem.ci_nit" label="CI/Nit"></v-text-field>
                    </v-col>
                    <v-col cols="12" sm="6" md="4">
                      <v-select
                        :items="paises"
      ...
      <template v-slot:item.actions="{ item }">
        ...
        <v-btn :to="`/admin/pedido/${item.id}`">Nuevo Pedido</v-btn>
      </template>
...
export default {
  ...
  created() {
    axios
      .get("http://localhost:8000/api/pais", { headers: getHeader() })
      .then(datos => {
  ...
  methods: {
    ...
    iniciar() {
      axios
        .get(base_url + "/cliente", { headers: getHeader() })
        .then(respuesta => {
    ...
    save() {
      if (this.editedIndex > -1) {
        ...
        axios.post(base_url + "/cliente", this.editedItem).then(respuesta => {
          if (respuesta.data.creado == false) {
            ...
            this.close();
            console.log(respuesta.data);
            Swal.fire("Cliente registrado correctamente", "Aceptar", "success");
          }
        });
